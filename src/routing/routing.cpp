...
// std::string buildPath(Server& server, Location& location, const std::string& request_path)
// {
...
//     if (root[root.size() - 1] == '/' && cleaned_path[0] == '/')
...
// bool isCgiExtension(const std::string& ext, const std::vector<std::string>& cgi_ext)
// {
...
// std::string findInterpreter(const std::string& ext, const std::vector<std::string>& cgi_ext, const std::vector<std::string>& cgi_path)
// {
...
// std::string generateDirectoryListing(const std::string& full_path, const std::string& request_path)
// {
...
// void setCgiEnvironment(Request& request, Server& server, const std::string& script_path)
// {
//     setenv("REQUEST_METHOD", request.getMethod().c_str(), 1);
//     setenv("SERVER_PROTOCOL", "HTTP/1.1", 1);
//     setenv("SERVER_PORT", toString(server.listen).c_str(), 1);
//     setenv("SERVER_NAME", server.server_name.c_str(), 1);
...
//     if (q != std::string::npos)
//     {
//         setenv("QUERY_STRING", full_path.substr(q + 1).c_str(), 1);
//         setenv("PATH_INFO", full_path.substr(0, q).c_str(), 1);
//     }
//     else
//     {
//         setenv("QUERY_STRING", "", 1);
//         setenv("PATH_INFO", full_path.c_str(), 1);
//     }
//     if (request.getMethod() == "POST")
//     {
//         setenv("CONTENT_LENGTH", toString(request.getBody().size()).c_str(), 1);
...
// Response handleCgi(Server& server, Location& location, Request& request, const std::string& script_path)
// {
//     Response response;
...
//         close(stdin_pipe[0]);
//         close(stdout_pipe[1]);
//         std::string cwd = script_path.substr(0, script_path.find_last_of('/'));
//         if (!cwd.empty())
//             chdir(cwd.c_str());
//         setCgiEnvironment(request, server, script_path);
...
//     if (!body.empty())
//     {
//         ssize_t written = write(stdin_pipe[1], body.c_str(), body.size());
//         (void)written;
//     }
//     close(stdin_pipe[1]);
...
//         cgi_body = cgi_output;
//     }
//     response._body = cgi_body;
//     response._contentType = content_type;
//     response._statusCode = status_code;
//     response._hasError = false;
//     return response;
// }

// Response routeRequest(Server& server, Request& request)
// {
//     Response response;
...
//         }
//         else if (!loc_path.empty() && loc_path[0] == '~')
//         {
//             is_regex = true;
//             is_prefix = false;
//         }
...
//     {
//         response._hasError = true;
//         response._errorCode = location.redirect_code;
//         response._redirectLocation = location.redirect;
//         return response;
//     }
...
//                 {
//                     response._body = generateDirectoryListing(full_path, request_path);
...
//     {
//         return handleCgi(server, location, request, full_path);
//     }
//     std::string content = readFile(full_path);
//     response._body = content;
//     response._contentType = getMimeType(full_path);
//     response._statusCode = 200;
//     response._hasError = false;
//     return response;
// }
